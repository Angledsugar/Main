...
int main (int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::Subscriber sub1 = nh.subscribe("/Status/IR", 1 ,IR);
    ros::Subscriber sub2 = nh.subscribe("/Status/Cam", 1 ,Cam);
    ros::Subscriber sub3 = nh.subscribe("/Status/Imu", 1 ,Imu);
    ros::Subscriber sub4 = nh.subscribe("/Status/Motor", 1 ,Motor);
    ...
    while (ros::ok())
    {
    	if(IR_ok & Cam_ok & Imu_ok & Motor_ok ){
    		digitalWrite(RED,LOW);
